...
export class FormularioComponent implements OnInit {
  ...
  valorSuscripcionTi(){
    ...
    if ( edadT < 65 ){
      ...
      this.status = 'error'
      this.statusText = 'El usuario es mayor de 73 años'
    }
  }
  valorSuscripcionBe(campo){
    let fechab = moment(campo)
    let edadb = parseInt(fechab.fromNow())
    console.log('la edad es: '+edadb)
    if ( edadb < 65 ){
      this.valorSuscripcionBeneficiario = 25000;
    }else if ( edadb > 64 && edadb < 70 ){
      this.valorSuscripcionBeneficiario = 30000;
    }else if ( edadb > 69 && edadb < 74 ){
  ...
  agregarBeneficiario() {
    ...
    if (this.formsBeneficiario.length <= 0) {
      ...
      if (this.validacionesFormularios() === 'valido') {
       ...
       this.valorSuscripcionBe(fechaBeneficiario)
